...
[Verfeinern, Menge für Sudokus, Menge für ausgefüllte Gitter, ,Menge für Lösungen]
...
Üblicherweise werden als Alphabet bei Größen bis 9 die arabischen Ziffern verwendet, für Größe 5 die römischen Buchstaben, und für größere Sudokus eine Mischung.
...
\subsection{Wie viele Sudokus gibt es?}

Das ist eine ziemlich interessante Frage. Man weiß aber die Antwort darauf bisher nicht.

Einfacher als die Frage nach der Anzahl der Sudokus ist die Anzahl der Lösungen. Für Lösungen der Größe 4 ist die Anzahl sogar recht einfach auszurechnen. Wir gehen blockweise vor. Zuerst der linke oberer Block. Es gibt $4!$ Möglichkeiten 4 Symbole auf 4 Plätze zu verteilen. Dadurch werden im oberen rechten und unteren linken Block einige der Möglichkeiten eliminiert. Es fehlen pro Zeile bzw. Spalte noch zwei Symbole, die man jeweils vertauschen kann. Also pro Block 4 Möglichkeiten. Der untere rechte Block ist damit schon vollständig festgelegt. Das macht insgesamt $4! \cdot 4 \cdot 4 = 384$ Lösungen. Sudokus der Größe 4 gibt es deutlich mehr, da zu jeder Lösung mehrere Sudokus existieren.

Für die Anzahl der Lösungen der Größe 9 kann man jedoch sehr grobe Abschätzungen anstellen. Es gibt $\frac{81!}{9!^9} = 5.3 \cdot 10^{70}$ Möglichkeiten, die 81 Symbole auf die 81 Felder zu verteilen. Jedoch sind nicht alle davon gültige Lösungen. Es handelt sich also um eine obere Grenze.

Als untere Grenze kann man sich überlegen, dass es ja mindestens eine Lösung gibt. Aus dieser Lösung lassen sich weitere Lösungen gewinnen, indem man gewisse Operationen durchführt. Zum Beispiel lassen sich die Symbole beliebig vertauschen, das ergibt schonmal $9! = 362880$ 'äquivalente' Möglichkeiten. Oder die Lösung lässt sich drehen und spiegeln, so kann man $4 \cdot 2 = 8$ unterschiedliche Varianten der Lösung gewinnen, wenn man davon ausgeht, dass die Lösung hinreichend asymetrisch ist. Des weiteren lassen sich Zeilen und Spalten innerhalb der Blockgrenzen beliebig vertauschen, pro Dreierpaket ergibt das $6$, insgesamt also $6^6 = 46656$ weitere Lösungen. Eine beliebige Kombination dieser Maßnahmen erlaubt es uns also, aus einer gegebenen Lösung (man könnte z.B. die zu dem Sudoku in Tabelle 1 nehmen) $9! \cdot 8 \cdot 6^6 = 1.354 \cdot 10^{11}$ weitere Lösungen zu erzeugen. Also gibt es mindestens so viele Lösungen. Das ist unsere untere Schranke.

Eine sehr grobe obere Schranke für die Anzahl der Sudokus kann man auch errechnen. Man weiß, dass ein Sudoku mit 17 festgelegten Symbolen eindeutig lösbar sein kann, mit 15 jedoch nicht mehr. Das heißt, dass es nur Sudokus mit zwischen 16 und 80 vorgegebenen Symbolen geben kann. Für $n \in {16,17,18,\dots 80}$ vorgegebene Symbolen gibt es höchstens $\frac{81!}{n!}$ Möglichkeiten, diese auf die 81 Plätze zu verteilen. Da ja auch immer Zahlen doppelt vorkommen, sind dadurch einige dieser Möglichkeiten gleich. Es sind mindestens $\cdot \lfloor \frac{n}{9} \rfloor ^{ 9 - ( n \% 9)} \cdot \lfloor \frac{n}{9} + 1 \rfloor ^ {n \% 9}$ dieser Möglichkeiten gleich, dieser Fall tritt ein, wenn die Symbolanzahlen möglichst gleich sind. Insgesamt bleiben dann höchstens

\begin{equation}
	Z = \frac{81!^{65}}{\prod_{i=16}^{65} i! \cdot \lfloor \frac{i}{9} \rfloor ^{ 9 - ( i \% 9)} \cdot \lfloor \frac{i}{9} + 1 \rfloor ^ {i \% 9}} \approx 6.6 \cdot 10^{3238}
\end{equation}

Möglichkeiten für Sudokus übrig.Dabei bedeutet $\lfloor k \rfloor$ die größte Ganzzahl, die kleiner oder gleichgroß ist wie $k$, und $a \% b$ bedeutet $a$ modulo $b$, also der Rest, der bei einer ganzzahligen Division übrigbleibt (wie in der Grundschule: $5 / 2 = 2$ Rest $1$).

\section{Lösungsverfahren}
In diesem Abschnitt werde ich verschiedene, zum Teil ausgefallene oder sehr abstrakte Lösungsmethoden vorstellen. Bei den meisten ist eine Implementierung in C oder Matlab beigefügt.
...
\label{BT}
Eine immer noch sehr aufwendige Methode ist die Backtrackingmethode. Zur Vereinfachung der Darstellung nummerieren wir die Symbole durch von 1 bis n. Man beginnt im ersten leeren Feld.
\begin{enumerate}
...
Folgendes C Programm implementiert diese Methode.
...
\subsection{Sudokus als Optimierungsproblem}
\label{OPT}
...
Zu einem gegebenen zu lösenden Sudoku
...
\subsection{spezielle Verfahren zur Lösung von quadratischen Programmen}

\subsection{Sudoku mit tanzenden Links}

\subsection{Neuronale Netze}

\section{Sudokugeneratoren}

\subsection{evolutionäre Programmierung}

\section{Schwierigkeit}

\section{weiterführende Literatur und Quellen}
